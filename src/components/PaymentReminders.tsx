  ...
  if (pending.length === 0 && recentlyPaid.length === 0) {
    return (
      <div className="rounded-2xl border border-border/50 bg-card p-5 shadow-card">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground"><Clock className="h-4 w-4" />Nenhum pagamento registrado.</div>
      </div>
    );
  }
      ...
      {pending.length > 0 && (
        <>
          <div className="mb-3 flex items-center justify-between">
            ...
            <span className="inline-flex items-center gap-1 rounded-full bg-warning/10 px-2.5 py-0.5 text-xs font-bold text-warning ring-1 ring-warning/20">
              <AlertTriangle className="h-3 w-3" />{pending.length} pendente{pending.length > 1 ? "s" : ""}
            </span>
          </div>
            ...
            {pending.map((e, i) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 ml-2">
                  ...
                  <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-profit hover:bg-profit/10 hover:text-profit rounded-lg" onClick={() => onMarkPaid(e.id)} title="Marcar como pago">
          ...
          <div className="mt-3 flex items-center justify-between border-t pt-3">
            ...
            <span className="text-sm font-bold tabular-nums text-warning">{totalPending.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</span>
          </div>
        </>
      )}
      ...
      {recentlyPaid.length > 0 && (
        <div className={pending.length > 0 ? "mt-4 pt-4 border-t" : ""}>
          ...
          <div className="space-y-1.5 max-h-36 overflow-y-auto">
            {recentlyPaid.map((e) => (
              <div key={e.id} className="flex items-center justify-between rounded-xl px-3 py-2 bg-profit/5 transition-colors hover:bg-profit/10">
                <div className="flex-1 min-w-0">
